 ...
 *
 * `id` придуман клиентом и стоит первым — как `stated` у [DrugEdit]: это не описание упаковки,
 * а часть протокола. Повтор запроса с тем же `id` упирается в первичный ключ, поэтому второй
 * упаковки от потерянного ответа не появится.
 */
data class NewDrug(
    val id: Uuid,
    val name: String,
