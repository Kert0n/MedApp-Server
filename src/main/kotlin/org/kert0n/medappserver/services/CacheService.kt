...
class CacheService(
    ...
    /**
     * Журнал расхода: какие синхронизации и приёмы уже применены, по придуманному клиентом
     * идентификатору.
     *
     * Не в базе: приём слишком персонален для таблицы. Даже обезличенная квитанция копила бы там
     * частоту действий, а на домашнем сервере с несколькими людьми это почти то же, что сам
     * журнал. Граница названа честно — журнал живёт в памяти процесса, значит не переживает
     * рестарт и не работает между экземплярами: повтор после рестарта спишет второй раз.
     *
     * Срок — верхняя граница, как и у ключей приглашения: при переполнении `maximumSize` вытеснит
     * запись раньше, и контракт обещает клиенту «не дольше суток», а не «сутки».
     */
    @Bean
