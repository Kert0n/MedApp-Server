...
/**
 * Оператор `<<%` из `pg_trgm`: у слова есть похожее **слово** внутри текста.
 *
 * Слово слева, текст справа — оператор несимметричен. Порог берётся из настройки
 * `pg_trgm.strict_word_similarity_threshold`, а не из выражения: это плата за то, что оператор
 * индексируемый, в отличие от сравнения с вызовом функции.
 *
 * Строгий вариант, а не `<%`, и вот почему. Нестрогий берёт лучший непрерывный участок триграмм
 * и границы слова не признаёт, а `search_text` — склейка четырёх полей. На ней участок собирается
 * из хвоста одного поля и начала другого: запросу «асприн» отвечал «норэпинефРИН АСПектус» с
 * оценкой 0.71 — выше, чем у настоящего слова «аспирин» с его 0.5. Так «Норадреналин» и
 * оказывался впереди «Аспирина». Строгий вариант обрывает участок на границе слова, и химера
 * получает 0.23.
 */
private class StrictWordSimilarOp(
    left: Expression<*>,
    right: Expression<*>
) : ComparisonOp(left, right, "<<%")

fun hasSimilarWord(word: String, text: Expression<*>): Op<Boolean> =
    StrictWordSimilarOp(stringParam(word), text)

/**
 * `strict_word_similarity(?, текст)` — насколько похоже на слово запроса ближайшее слово текста.
 *
 * Не `similarity`: та сравнивает строки целиком, и короткое слово против склейки четырёх полей
 * даёт почти ноль. Здесь ищется лучшее слово, поэтому длина записи ответу не мешает — и поэтому
 * же лишнее слово в запросе не прячет нужную запись.
 *
 * Та же метрика, что у [StrictWordSimilarOp]: отбор и подсчёт очков обязаны мерить одним, иначе
 * запись попадает в выдачу тем, что при подсчёте не засчитано.
 */
fun wordSimilarity(word: String, text: Expression<*>): CustomFunction<Float?> =
    CustomFunction("strict_word_similarity", FloatColumnType(), stringParam(word), text)
