 ...
 *
 * Оба правила ниже нарушаются незаметно: хранилище оказывается видно оркестратору, а контроллер
 * начинает разговаривать то с прикладным сервисом, то с сервисом агрегата — выбирая по признаку
 * «сколько агрегатов трогает операция», которого из HTTP не видно.
 *
 * Проверяется по исходникам, а не рефлексией: правило про **импорты**, а рефлексия увидела бы
 * только то, что осталось в байт-коде. Полный список правил части 2 и того, чем каждое
 * закреплено, — в `ARCHITECTURE-RULES.md`.
 */
