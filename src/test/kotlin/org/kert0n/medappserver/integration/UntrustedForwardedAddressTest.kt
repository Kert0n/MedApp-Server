 ...
 *
 * Лимит регистраций считается по адресу клиента, и если заголовку верить от кого угодно, он
 * обходится сменой значения. Защита держится на том, что RemoteIpValve подставляет адрес из
 * заголовка **только** для peer'ов из доверенных диапазонов.
 *
 * Здесь диапазон сужен так, что localhost в него не входит, — тестовый клиент недоверенный.
 * Через настоящий HTTP: заголовок разбирает Tomcat, MockMvc его не задействует. Профиль test
 * задаёт BanNumber=1, а `validateRequest` сравнивает через `<=`, поэтому с адреса проходят две
 * регистрации, третья отклоняется.
 */
@SpringBootTest(
    webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT,
    properties = [
        "server.forward-headers-strategy=native",
        // Доверены только 10.0.0.0/8, поэтому петля недоверенная. Без этой строки петля
        // попадает в список по умолчанию, заголовку верят и тест падает.
