...
class DatabaseTestHelper(
    ...
    /**
     * Ничего не делает: синхронизировать нечего, запрос в Exposed всегда идёт в базу.
     *
     * Пустышка держится только тем, что её зовут из 98 мест. Убрать её — отдельная механическая
     * правка; пока она здесь, вызов в тесте ничего не значит и читать его как «здесь важна
     * синхронизация» не надо.
     */
     ...
     *
     * В приложении такого чтения нет: там доступ проверяется всегда. Тесту же нужно убедиться,
     * что строки не стало, безотносительно того, кто спрашивает.
     */
    @Transactional
