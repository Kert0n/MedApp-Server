 ...
 *
 * Своей копии операторов здесь нет намеренно: копия отстаёт молча — тесты идут по базе без
 * индекса по `search_tsv`, и полнотекстовая ступень поиска проверяется не та.
 */
@Component
class TestSchema {
