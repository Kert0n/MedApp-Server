 ...
 *
 * `JdbcTemplate` в проде запрещён отдельно. Правило стало выполнимым только что: нечёткий
 * поиск перестал спускаться к драйверу, и последнее обращение осталось в тестах.
 */
class NoPessimisticLockingTest {
     ...
     *
     * Это осознанно. Написать «здесь мог бы быть FOR UPDATE» и оставить — ровно тот способ,
     * которым запрет размывается; сообщение называет файл и строку, так что разобраться легко.
     */
