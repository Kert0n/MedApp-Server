...
N+1-гейты используют штатную `Statistics` Hibernate. Основной счётчик —
`prepareStatementCount`; рядом проверяются `queryExecutionCount`, `entityFetchCount` и
`collectionFetchCount`. Fixture создаётся до измерения, persistence context очищается, а
