...
Для существующей базы каскадные внешние ключи Drug/Using/MedKit обновляются идемпотентным
скриптом:
...
Скрипт можно выполнять повторно. После него приложение проверяет итоговую схему через
`ddl-auto=validate`. Для одноразовой локальной базы допустимо пересоздание тома с
