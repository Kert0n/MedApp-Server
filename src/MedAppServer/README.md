...
## Первый запуск: ключи подписи JWT

Ключи не хранятся в репозитории и не попадают в jar или образ — это runtime-вход.
На чистом клоне их надо создать один раз:

```bash
src/main/resources/certs/gen.sh
```

Скрипт пишет пару в `.local/secrets/` (git-ignored), запускается из любого каталога и
повторный вызов ничего не меняет. Ротация — только явно, через `--force`: она
инвалидирует все уже выданные токены.

В проде путь к ключам задаётся переменными `RSA_PRIVATE_KEY_LOCATION` и
`RSA_PUBLIC_KEY_LOCATION` (это **пути к файлам**, а не содержимое ключа), см.
[application-prod.example.properties](src/main/resources/application-prod.example.properties).
