...
Схема лежит в [db/schema.sql](db/schema.sql) — это генерируемый файл, источник истины —
JPA-сущности. Он даёт путь от пустой БД к рабочей схеме: применяется при инициализации
Postgres в compose, поэтому на чистой машине ничего делать не нужно.

Flyway сознательно не используется. Но `ddl-auto=update` **никогда не удаляет и не меняет
существующие колонки**, только добавляет, поэтому такие изменения оформляются отдельными
скриптами в [db/manual/](db/manual/README.md) и применяются с остановкой сервера. Там же
журнал применённого на проде.
