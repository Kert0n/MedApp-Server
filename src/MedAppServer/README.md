...
Drug, TreatmentPlan и MedKit изменяются отдельными транзакционными командами. Чтение
возвращает неизменяемые проекции, а команды над несколькими агрегатами находятся в
`services/orchestrators`.

Таблица разрешённых связей между пакетами закреплена тестом `ArchitectureTest` — нарушение
роняет сборку. Границы агрегатов, порядок блокировок и SQL-каскады описаны в
...
Изменение схемы применяется пересозданием базы: правится `db/schema.sql`, том удаляется,
а Postgres заново создаёт структуру и загружает справочник.
...
Пока приложение не хранит пользовательские данные, `db/schema.sql` остаётся единственным
описанием схемы. Отдельные SQL-миграции в текущем жизненном цикле не применяются.
...
Скраппер отдаёт `pg_dump` таблиц `drugs`, `form_types` и `quantity_units`. Перед загрузкой
его нужно преобразовать в data-only файл, совместимый со структурой `db/schema.sql`:

```bash
python3 db/rewrite-catalogue-dump.py init-scripts/cleaned-init.sql
```

Скрипт переписывает файл на месте (исходник остаётся рядом с суффиксом `.orig`) и создаёт
три `COPY`: сначала справочники, затем `parsed_drugs`. Загрузчик распознаёт исходный
структурный dump и требует сначала преобразовать его.
...
Все прикладные пути начинаются с `/v1`. Проект pre-release: устаревшие маршруты
`/v1/drug`, `/v1/using`, `/v1/med-kit` и `/v1/user` не имеют alias.
...
- `GET /v1/med-kits` — список аптечек
- `POST /v1/med-kits` — создать аптечку
- `GET /v1/med-kits/{medKitId}` — содержимое аптечки
- `DELETE /v1/med-kits/{medKitId}` — удалить аптечку
- `POST /v1/med-kits/{medKitId}/invitations` — создать приглашение
- `POST /v1/med-kit-memberships` — присоединиться по приглашению
- `DELETE /v1/med-kit-memberships/{medKitId}` — покинуть аптечку

### Препараты

- `GET /v1/drugs/{drugId}` — получить доступный препарат
- `POST /v1/med-kits/{medKitId}/drugs` — создать препарат
- `PATCH /v1/drugs/{drugId}` — исправить данные или увеличить остаток
- `DELETE /v1/drugs/{drugId}` — удалить препарат
- `POST /v1/drugs/{drugId}/consumptions` — списать остаток
- `PUT /v1/med-kits/{targetMedKitId}/drugs/{drugId}` — перенести препарат
- `GET /v1/drug-templates` — поиск по справочнику
- `GET /v1/drug-templates/{templateId}` — карточка справочника

### Планы лечения

- `GET /v1/treatment-plans` — собственные планы
- `POST /v1/treatment-plans` — создать план
- `GET /v1/treatment-plans/{drugId}` — получить собственный план препарата
- `PATCH /v1/treatment-plans/{drugId}` — изменить план
- `DELETE /v1/treatment-plans/{drugId}` — удалить план
- `PUT /v1/intakes/{intakeId}` — идемпотентно зарегистрировать приём
