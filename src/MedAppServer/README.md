...
`ddl-auto=update` не удаляет и не меняет существующие колонки, только добавляет, поэтому
такие изменения применяются отдельным SQL с остановкой сервера, а `db/schema.sql`
