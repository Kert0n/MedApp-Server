...
data class Reservation(
    ...
    /**
     * Переживает ли назначение переезд пачки в другое хранилище.
     *
     * Аптечка — физическое хранилище, а бронь — назначение. Коробку переставили на другую
     * полку: если человек к этой полке допущен, назначение никуда не делось; если нет — пачки
     * он больше не видит, и назначение на неё смысла не имеет.
     *
     * Правило написано здесь, а не только в запросе, который его исполняет: искать его будут
     * тут. Массовое удаление в хранилище — та же мысль на SQL, и согласие проверяется тестом.
     */
    fun survivesRelocationTo(target: MedKit): Boolean = target.isMember(userId)
