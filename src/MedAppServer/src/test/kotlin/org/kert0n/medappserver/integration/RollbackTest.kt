 ...
 *
 * Вторая половина файла — про отказ, который наступает **в конце**. Предъявленная версия
 * сверяется теперь предикатом записи, то есть в последнюю очередь: к этому моменту команда
 * успела переставить упаковки, снять чужие брони, уничтожить пачку. Проверяется не сам отказ —
 * он под тестом и так, — а что после него не осталось ничего.
 */
...
class RollbackTest {
     ...
     * пережил. Раз отказ наступает теперь позже, это надо доказать: повтор с тем же
     * идентификатором обязан выполнить команду, а не ответить «уже применено».
     */
    @Test
