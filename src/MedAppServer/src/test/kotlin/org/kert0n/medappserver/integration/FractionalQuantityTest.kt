...
class FractionalQuantityTest {
    ...
    fun `потребление всего остатка дробными долями удаляет препарат и план`() {
        ...
        usingService.recordIntake(user.id, drug.id, third)
        usingService.recordIntake(user.id, drug.id, third)
        ...
        val afterLast = usingService.recordIntake(user.id, drug.id, last)
    ...
    fun `инвариант держится после пропорционального уменьшения планов`() {
        ...
        usingService.recordIntake(alice.id, drug.id, consumed)
        ...
        assertQty(
            remaining,
            plansTotal,
            "сумма планов должна ровно равняться остатку: остаток округления отдаётся одному плану"
        )
    }
}
