...
class ReservationSnapshotTest {
    ...
    private val drugId = Uuid.random()
    ...
    fun `без своей брони доля отсутствует, а не равна нулю`() {
        val snapshot = ReservationSnapshot.of(drugId, listOf(reservation(bob, "5.5")), alice, version = 1)
    ...
    fun `пустой снимок несёт версию`() {
        val snapshot = ReservationSnapshot.empty(drugId, version = 7)
