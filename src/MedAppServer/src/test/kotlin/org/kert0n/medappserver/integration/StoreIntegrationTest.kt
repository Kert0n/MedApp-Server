...
class StoreIntegrationTest {
    ...
    fun `упаковки аптечки читаются без броней — упаковка о них не знает`() {
        ...
        reservationService.create(drugService.require(first.id, alice.id), alice.id, qty(4.0))
    ...
    fun `планы участника собираются по всем препаратам`() {
        ...
        reservationService.create(drugService.require(first.id, alice.id), alice.id, qty(5.0))
        reservationService.create(drugService.require(second.id, alice.id), alice.id, qty(7.0))
        dbHelper.flushAndClear()
    ...
    fun `массовый перенос повторяет правило переезда препарата`() {
        ...
        medKitService.joinByInvitation(medKitService.invite(medKitService.require(source.id, alice.id), alice.id), bob.id)
        ...
        reservationService.create(drugService.require(first.id, alice.id), alice.id, qty(10.0))
        reservationService.create(drugService.require(first.id, bob.id), bob.id, qty(20.0))
        dbHelper.flushAndClear()
