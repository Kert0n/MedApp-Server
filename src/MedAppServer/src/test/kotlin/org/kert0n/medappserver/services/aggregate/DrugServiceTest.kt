...
class DrugServiceTest {
    ...
    fun `update increasing quantity bypasses reduction`() {
        ...
        drugService.update(drug.id, DrugEdit(quantity = qty(20.0)), alice.id, dbHelper.drugVersion(drug.id))
        dbHelper.flushAndClear()
    ...
    fun `update decreasing quantity leaves reservations alone`() {
        ...
        drugService.update(drug.id, DrugEdit(quantity = qty(50.0)), alice.id, dbHelper.drugVersion(drug.id))
        dbHelper.flushAndClear()
    ...
    fun `update refuses a non-positive quantity`() {
        ...
        assertThrows<InvalidQuantity> {
            drugService.update(drug.id, DrugEdit(quantity = qty(0.0)), alice.id, dbHelper.drugVersion(drug.id))
        }
    }
