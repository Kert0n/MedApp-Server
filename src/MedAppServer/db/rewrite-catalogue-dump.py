...
Вход содержит COPY таблиц `drugs`, `form_types` и `quantity_units`. Выход содержит только
...
Файл переписывается на месте, исходный pg_dump сохраняется с суффиксом `.orig`. Data-only
файл распознаётся по отсутствию COPY таблицы `drugs` и повторно не изменяется.
