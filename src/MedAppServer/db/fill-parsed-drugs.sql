...
-- Без этого поиск остаётся медленным до первого прихода autovacuum.
--
-- Таблицу только что наполнили 18 тысячами строк, статистики по ней нет, и планировщик
-- выбирает последовательное сканирование вместо девяти индексов. Замерено на реальных
-- данных: 47 мс до ANALYZE и 2 мс после. Одна строка здесь избавляет от «первые запросы
-- почему-то тормозят», которое иначе пришлось бы ловить в проде.
ANALYZE parsed_drugs;
